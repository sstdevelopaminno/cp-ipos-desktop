...
Receipt printing note: receipt output is rendered as a monochrome raster image in the frontend and sent to the Windows spooler with the Tauri `print_receipt_raster` command. This keeps Thai text, logo placement, and layout closer to the on-screen receipt preview than text/codepage printing. The print payload includes extra white feed space plus an ESC/POS feed command before cutting so the footer and thank-you line are not cut off by 80mm printer cutters. Sales checkout auto-print and receipt-history reprint both use the same `src/receipt-print.ts` helper.
