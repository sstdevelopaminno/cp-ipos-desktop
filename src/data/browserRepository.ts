...
const DEFAULT_STAFF: Staff & { demoPin?: string } = { id:"staff-owner", code:"OWNR", displayName:"ผู้ดูแลร้าน", role:"owner", active:true, demoPin:"1234" };
...
export class BrowserRepository implements PosRepository {
  async initialize() { if(!localStorage.getItem(K.products)) write(K.products, DEMO_PRODUCTS); if(!localStorage.getItem(K.staff)) write(K.staff, [DEFAULT_STAFF]); else { const employees=read<Array<Staff & {demoPin?:string}>>(K.staff,[DEFAULT_STAFF]); if(!employees.some(e=>e.id!=="staff-owner" && e.code.toLowerCase()==="ownr")) write(K.staff, employees.map(e=>e.id==="staff-owner" && e.code.length>4 ? {...e,code:"OWNR",demoPin:e.demoPin || "1234"}:e)); } if(!localStorage.getItem(K.settings)) write(K.settings, DEFAULT_SETTINGS); }
  async verifyPin(pin:string, code?:string) { const employees=read<Array<Staff & {demoPin?:string}>>(K.staff,[DEFAULT_STAFF]); const wanted=code?.trim().toLowerCase(); return employees.find(e=>e.active!==false && (!wanted || e.code.toLowerCase()===wanted) && (e.demoPin || (e.id==="staff-owner" ? "1234" : ""))===pin) || null; }
  ...
  async saveEmployee(input:EmployeeInput, staff:Staff) { if(!["owner","manager"].includes(staff.role)) throw new Error("EMPLOYEE_PERMISSION_DENIED"); const id=input.id || crypto.randomUUID(); const code=input.code.trim().toUpperCase(); const displayName=input.displayName.trim(); if(!code || code.length>4) throw new Error("EMPLOYEE_CODE_TOO_LONG"); if(input.demoPin && !/^\d{4}$/.test(input.demoPin.trim())) throw new Error("EMPLOYEE_PIN_INVALID"); const employees=read<Array<Staff & {demoPin?:string}>>(K.staff,[DEFAULT_STAFF]); if(employees.some(e=>e.id!==id && e.code.toLowerCase()===code.toLowerCase())) throw new Error("EMPLOYEE_CODE_EXISTS"); const current=employees.find(e=>e.id===id); const emp={id,code,displayName,role:input.role,active:input.active,demoPin:input.demoPin?.trim() || current?.demoPin}; const all=employees.filter(e=>e.id!==emp.id); write(K.staff,[emp,...all]); await this.audit(input.id?"EMPLOYEE_UPDATED":"EMPLOYEE_CREATED", staff, {entityType:"employee",entityId:emp.id}); return emp; }
