...
export class BrowserRepository implements PosRepository {
  ...
  async checkout(input:CheckoutInput) { const total=money(input.items.reduce((s,i)=>s+qty(i.quantity)*money(i.unitPrice),0)); if(!input.items.length) throw new Error("EMPTY_CART"); if(input.paymentMethod==="cash" && money(input.paid)<total) throw new Error("INSUFFICIENT_CASH"); const paid=money(input.paymentMethod==="cash"?input.paid:total); const sale:Sale & Partial<Pick<Receipt,"subtotal"|"discountAmount"|"discountType"|"discountValue">>={id:crypto.randomUUID(),receiptNo:`R${Date.now().toString().slice(-8)}`,total,paid,changeAmount:money(Math.max(0,paid-total)),paymentMethod:input.paymentMethod,createdAt:new Date().toISOString(),status:"completed",cashierName:input.staff.displayName,employeeCode:input.staff.code,shiftId:input.shift.id,subtotal:input.subtotal,discountAmount:input.discountAmount,discountType:input.discountType,discountValue:input.discountValue}; const products=await this.listProducts(); write(K.products,products.map(p=>{ const line=input.items.find(i=>i.productId===p.id); return line?{...p,stockQuantity:qty(p.stockQuantity-line.quantity)}:p; })); write(K.sales,[sale,...await this.listSales(999)]); write(K.items,{...read<Record<string,unknown[]>>(K.items,{}),[sale.id]:input.items}); await this.audit("SALE_COMPLETED", input.staff, {entityType:"sale",entityId:sale.id,shiftId:input.shift.id,status:input.paymentMethod,details:JSON.stringify({subtotal:input.subtotal,discountAmount:input.discountAmount,discountType:input.discountType,discountValue:input.discountValue})}); return sale; }
