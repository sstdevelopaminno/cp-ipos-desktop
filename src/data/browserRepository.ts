...
export class BrowserRepository implements PosRepository {
  async initialize() { if(!localStorage.getItem(K.products)) write(K.products, DEMO_PRODUCTS); if(!localStorage.getItem(K.staff)) write(K.staff, [DEFAULT_STAFF]); if(!localStorage.getItem(K.settings)) write(K.settings, DEFAULT_SETTINGS); }
